...
print()
gameNum = int(input("Enter game number: "))
replaySpeed = int(input("Enter replay speed (1-4): "))
with open('replay.json') as json_file:
    dataa = json.load(json_file)
    if gameNum > len(dataa):
            print("Game number does not exist.")
            exit()  
    data = dataa[gameNum-1]
    blank_board = []
    for i in range(ROWS):
        row = ['_']*COLS
        row.append('\n')
