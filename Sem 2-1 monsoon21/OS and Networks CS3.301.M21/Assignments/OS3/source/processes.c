...
//lists background processes according to given options (-r, -s)
void listProc(char** inpt)
{
	...
	for(int i = 0; inpt[i] != NULL; i++)
	{
		if(inpt[i][0] == '-')
		{
			if(strchr(inpt[i], 'r'))
				r = 1;
			else if (strchr(inpt[i], 's'))
				s = 1;
		}
	}
	//no options were given then print all
	if(r == -1 && s == -1)
	{
		r = 1;
		s = 1;
	...
	for(int i = 0; procList[i] != NULL; i++)
	{
		if((strcmp(procList[i]->status, "Stopped") == 0 && s == 1) || (strcmp(procList[i]->status, "Running") == 0 && r == 1))
		{
			printf("[%d] %s %s [%d]\n",
				procList[i]->number,
				procList[i]->status,
...
//change status of a proc to 0-running 1-stopped 2-foreground
...
//adds an item to the process list
