...
found:
  ...
  // initialise tracing array with zeroes
  memset(&p->trac, 0, sizeof(p->trac));
  // set the creation time, sleeping time and running time
  p->ctime = ticks;
  p->sleptAt = 0;
  p->stime = 0;
  p->rtime = 0;
  p->etime = 0;
  // set static priority to 60 by default
  p->SP = 60;
...
fork(void)
{
  ...
  memmove(p->trac, np->trac, sizeof(p->trac));
      ...
      if(np->parent == p){
        ...
        if(np->state == ZOMBIE){
          ...
          *wtime = np->etime - np->ctime - np->rtime;
          printf("rtime = %d, wtime = %d", *rtime, *wtime);
          if(addr != 0 && copyout(p->pagetable, addr, (char *)&np->xstate,
                                  sizeof(np->xstate)) < 0) {
            release(&np->lock);
            release(&wait_lock);
...
// set_priority syscall definition
int
set_priority(int prio, int pid)
{
  struct proc* p;
  int retur = 0;
  for(p = proc; p < &proc[NPROC]; p++)
...
// used to update the ticks in each process (used for PBS mainly)
void
updateProcTicks()
{
  struct proc* p;
